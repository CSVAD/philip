...
public class DShape implements DObj{
	...
	public ArrayList<DPoint> getNormalizedVertices(){
	  ...
	  public void addVertex(float x,float y){
		  ...
		  DPoint v = new DPoint(x,y); // create a point at this new coord
		  int lastVertex = this.lastVertex();
		  
		  if(lastVertex > -1){ // if last vertex EXISTS:
			  // fetch the last vertex recorded
			  ...
			  // push matrix, render transformations
			  myParent.pushMatrix();
			  for(int i = 0;i<this.objects.size();i++){
				  this.objects.get(i).renderTransformation(myParent);
			  }
			  
			  // draw the line from last vertex to this one
	  ...
	  public void render() {
		  ...
		  for(int v = 0; v<this.vertices.size(); v++) {
			  if(v > 0) {
				  ...
				  DPoint current = (DPoint)this.vertices.get(v);
				  DPoint last = (DPoint)this.vertices.get(v-1);
				  myParent.line(last.x, last.y, current.x, current.y);
			  }
		  }
	  }
